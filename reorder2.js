...
// Read the restored content.js
let code = fs.readFileSync('content.js', 'utf8');

// We need to extract the "entrenamiento-placa-pruebas" from it.
let idxPlaca = code.indexOf('    {\n\n      id: "entrenamiento-placa-pruebas"');
let endIdx = code.indexOf('  ]\n};\n');

// The chunk for placa is from idxPlaca to endIdx - 4 (excluding `    }\n  ]\n};\n`)
let chunkPlaca = code.substring(idxPlaca, endIdx);
chunkPlaca = chunkPlaca.trimEnd();

// The rest of the courses starts at the beginning of courses array up to idxPlaca
let startIdx = code.indexOf('courses: [\n') + 'courses: [\n'.length;
// We need to strip the comma before idxPlaca
let restCourses = code.substring(startIdx, idxPlaca);
restCourses = restCourses.trimEnd();
if (restCourses.endsWith(',')) {
    restCourses = restCourses.slice(0, -1);
}

// The electronica discreta chunk
